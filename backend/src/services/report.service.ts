...
export const updateReportSettingService = async (
  ...
  const { isEnabled, frequency } = body; // Ambil 'frequency' dari body permintaan
  let nextReportDate: Date | null = null;

  const existingReportSetting = await ReportSettingModel.findOne({ userId });
  if (!existingReportSetting) {
    throw new NotFoundException("Pengaturan laporan tidak ditemukan");
  }

  // Gunakan frekuensi baru jika ada, jika tidak, gunakan yang sudah ada
  const newFrequency = frequency || existingReportSetting.frequency;
