...
export default function AssetsPage() {
    // 1. Menggunakan Hook RTK Query untuk mengambil data aset
    const { data, isLoading, isError } = useGetAllAssetsQuery();

    // 2. Mengambil saldo dari transaksi user (real-time)
    const { data: analyticsData, isLoading: isAnalyticsLoading } = useSummaryAnalyticsQuery({});

    const assets = data?.data?.assets || [];
    const totalAssetValue = data?.data?.totalNetWorth || 0;

    // Saldo dari transaksi (pemasukan - pengeluaran)
    const userBalance = analyticsData?.data?.availableBalance || 0;

    // Total Kekayaan Bersih = Saldo Transaksi + Total Nilai Aset
    const totalNetWorth = userBalance + totalAssetValue;

    // Hitung Total per Kategori untuk Grafik (termasuk saldo sebagai "kas")
    const cashFromAssets = assets
        .filter((a) => a.category === 'cash')
        .reduce((sum, a) => sum + a.amount, 0);

    const allocationData = [
        // Saldo User (dari transaksi) + Kas dari Aset
        {
            name: "Saldo & Kas",
            value: userBalance + cashFromAssets,
            color: CATEGORY_COLORS.cash,
        },
        // Investasi
        {
            name: CATEGORY_LABELS.investment,
            value: assets
                .filter((a) => a.category === 'investment')
                .reduce((sum, a) => sum + a.amount, 0),
            color: CATEGORY_COLORS.investment,
        },
        // Properti
        {
            name: CATEGORY_LABELS.property,
            value: assets
                .filter((a) => a.category === 'property')
                .reduce((sum, a) => sum + a.amount, 0),
            color: CATEGORY_COLORS.property,
        },
        // Kripto
        {
            name: CATEGORY_LABELS.crypto,
            value: assets
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="grid grid-cols-1 gap-4">
                    ...
                    <StatCard
                        ...
                        amount={(allocationData.find(d => d.name === CATEGORY_LABELS.investment)?.value || 0) + (allocationData.find(d => d.name === CATEGORY_LABELS.property)?.value || 0) + (allocationData.find(d => d.name === CATEGORY_LABELS.crypto)?.value || 0)}
                        icon={Building2}
