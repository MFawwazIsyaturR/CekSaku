...
const ConfirmationStep = ({
  ...
  const handleImport = () => {
    ...
    if (transactions.length > MAX_IMPORT_LIMIT) {
      toast.error(`Tidak dapat impor lebih dari ${MAX_IMPORT_LIMIT} transaksi`);
      return;
    }
    resetProgress();
    startProgress(10);
    ...
    bulkImportTransaction(payload)
      ...
      .catch((error) => {
        resetProgress();
        toast.error(error.data?.message || "Gagal mengimpor transaksi");
      })
      .finally(() => {
  ...
  const getAssignFieldToMappedTransactions = () => {
    ...
    csvData.forEach((row, index) => {
      ...
      Object.entries(mappings).forEach(([csvColumn, transactionField]) => {
        ...
        console.log(row, csvColumn, transactionField), "csvColumn";
  ...
  return (
    <div className="space-y-6">
      <DialogHeader>
        ...
        <DialogDescription>
          Perhatikan kembali setelan Anda sebelum mengimpor
        </DialogDescription>
      </DialogHeader>
      ...
      <div className="space-y-4">
        <div className="border rounded-md p-4 w-full">
          ...
          <div className="grid grid-cols-2 w-full gap-4 text-sm">
            ...
            <div>
              <p className="text-muted-foreground">Kolom Telah Dipetakan</p>
            ...
            <div>
              <p className="text-muted-foreground">Batas Transaksi </p>
      ...
      <div className="flex justify-between">
        ...
        <Button onClick={handleImport} disabled={isLoading}>
          {isLoading ? "Mengimpor..." : "Konfirmasi Impor"}
        </Button>
      </div>
    </div>
  );
